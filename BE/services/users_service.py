...
# def create_user_oauth(db: Session, user: UserCreateOauth) -> str:
#   db_user = users_repository.create_user_oauth(db, user)

#   if not db_user:
#     raise HTTPException(status_code=400, detail="User creation failed")

#   token = create_jwt_token(getattr(db_user, "id"))
#   create_default_job_cycle(db, getattr(db_user, "id"))
#   return token
